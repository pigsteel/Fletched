		...
		if (entity instanceof Player player) {
			...
			if (projectile.isEmpty()) {
				...
				if ((double)pow < 0.05) {
					...
					level.playSound((Entity)null, player.getX(), player.getY(), player.getZ(), SoundEvents.ARROW_SHOOT, SoundSource.PLAYERS, 1.0F, 0.5F / (level.getRandom().nextFloat() * 0.4F + 1.2F) + pow * 0.5F);
